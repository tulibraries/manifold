...
export default class extends Controller {
  ...
  connect() {
    ...
    console.log("Add button target:", this.addBtnTarget)
    console.log("Remove button target:", this.removeBtnTarget)
    
    this.currentRequestCount = 1
    this.maxRequests = 10
    
    console.log("Initialized with count:", this.currentRequestCount)
  }

  addRequest() {
    console.log("Add request button clicked")
    console.log("Current request count:", this.currentRequestCount)
    console.log("Max requests:", this.maxRequests)
    
    if (this.currentRequestCount < this.maxRequests) {
      console.log("Looking for data-index:", this.currentRequestCount)
      const nextGroup = this.containerTarget.querySelector(`[data-index="${this.currentRequestCount}"]`)
      console.log("Found next group:", nextGroup)
