...
echo "Running: pipenv run ansible-playbook -i inventory/qa playbook.yml --vault-password-file=~/.vault -e rails_app_git_branch=main -vv"
pipenv run ansible-playbook -i inventory/stage playbook.yml --vault-password-file=~/.vault -e rails_app_git_branch=main -vv
