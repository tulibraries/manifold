class Ability
  ...
  def initialize(account)
		can :read, [Building, Space, Group, Person, Alert] # permissions for every account, even if not logged in 
    if account.present?  # additional permissions for logged in accounts (they can manage their posts)
      ...
      can :manage, [Building, Space, Person, Group, Alert], account_id: account.id
