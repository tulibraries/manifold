...
RSpec.describe ServiceGroup, type: :model do
  ...
  let(:group1) { FactoryBot.create(:group, persons: [person], space: space, chair_dept_heads: [person]) }
  let(:group2) { FactoryBot.create(:group, persons: [person], space: space, chair_dept_heads: [person]) }
